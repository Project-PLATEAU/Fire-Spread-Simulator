...
namespace SimulationSourceFileCreator.Model
{
    ...
    /// 要素追加設定ファイルのモデルクラス
    /// </summary>
    public class ElementAddSetting
    {
        /// <summary>
        /// 取得要素設定（建物IDの設定）
        /// </summary>
        ...
        /// </summary>
        public ElementAddSettingSupplementItem KOZO { get; set; } = new ElementAddSettingSupplementItem();

        /// <summary>
        /// 取得要素設定（データ補完対象項目）MOKU
        /// </summary>
        public ElementAddSettingSupplementItem MOKU { get; set; } = new ElementAddSettingSupplementItem();

        /// <summary>
        /// 取得要素設定（データ補完対象項目）YOTO
        /// </summary>
        public ElementAddSettingSupplementItem YOTO { get; set; } = new ElementAddSettingSupplementItem();
        ...
        internal static void Initialize(string configFilePath)
        {
            ...
            var resourceName = "SimulationSourceFileCreator.Resources.ElementAddSetting.xml";
        ...
        internal static ElementAddSetting? Load(string configFilePath)
        {
            if (!File.Exists(configFilePath))
            {
            ...
            ElementAddSetting? setting = null;
            var serializer = new XmlSerializer(typeof(ElementAddSetting));

            try
            {
                using (var reader = new StreamReader(configFilePath))
                {
                    setting = serializer.Deserialize(reader) as ElementAddSetting;
                }
            }
            catch (Exception ex)
            {
