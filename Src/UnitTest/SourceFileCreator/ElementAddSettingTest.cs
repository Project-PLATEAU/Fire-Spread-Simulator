...
/// <summary>
/// ElementAddSettingテストクラス
/// </summary>
[TestClass]
public class ElementAddSettingTest
{
    /// <summary>
    /// テストデータフォルダパス
    /// </summary>
    private static string TestDataFolder => Path.Combine(@"..\..\..\SourceFileCreator", "TestData", "ElementAddSettingTest");

    /// <summary>
    /// <see cref="ElementAddSetting.Load"/>のテスト（ファイル）
    /// </summary>
    ...
    [DataRow("ElementAddSetting_dummy.xml", false, DisplayName = "ファイルがない")]
    [DataRow("ElementAddSetting_ファイル確認_タグ不正.xml",       false)]
    [DataRow("ElementAddSetting_ファイル確認_空.xml",             false)]
    [DataRow("ElementAddSetting_ファイル確認_正常（初期値）.xml", true)]
    public void LoadTest_ForFile(string fileName, bool expectedValid)
    {
        ...
        var setting = ElementAddSetting.Load(configFilePath);
    ...
    /// <summary>
    /// <see cref="ElementAddSetting.Load"/>のテスト（内容）
    /// </summary>
    /// <param name="fileName">ファイル名</param>
    /// <param name="expectedValid">期待値：ファイルが有効かどうか</param>
    [TestMethod]
    [DataRow("ElementAddSetting_内容確認_すべて空.xml",                                     false)]
    [DataRow("ElementAddSetting_内容確認_建物IDの設定なし.xml",                             false)]
    [DataRow("ElementAddSetting_内容確認_取得要素設定（KOZOデータ補完）1_耐火構造なし.xml", false)]
    [DataRow("ElementAddSetting_内容確認_取得要素設定（KOZOデータ補完）2_建物構造なし.xml", false)]
    [DataRow("ElementAddSetting_内容確認_取得要素設定（KOZOデータ補完）3_地上階数なし.xml", false)]
    [DataRow("ElementAddSetting_内容確認_取得要素設定（KOZOデータ補完）4_延床面積なし.xml", false)]
    [DataRow("ElementAddSetting_内容確認_取得要素設定（KOZOデータ補完）5_建築面積なし.xml", false)]
    [DataRow("ElementAddSetting_内容確認_取得要素設定（KOZOなし）.xml",                     false)]
    [DataRow("ElementAddSetting_内容確認_取得要素設定（MOKUデータ補完）建物構造なし.xml",   false)]
    [DataRow("ElementAddSetting_内容確認_取得要素設定（MOKUなし）.xml",                     false)]
    [DataRow("ElementAddSetting_内容確認_取得要素設定（YOTOデータ補完）利用目的なし.xml",   false)]
    [DataRow("ElementAddSetting_内容確認_取得要素設定（YOTOなし）.xml",                     false)]
    [DataRow("ElementAddSetting_内容確認_取得要素設定のKeyNameが重複.xml",                  false)]
    [DataRow("ElementAddSetting_内容確認_追加要素設定_AddParentElementなし.xml",            false)]
    [DataRow("ElementAddSetting_内容確認_追加要素設定_AddParentElement空.xml",              false)]
    [DataRow("ElementAddSetting_内容確認_追加要素設定_SimNamespaceUriなし.xml",             false)]
    [DataRow("ElementAddSetting_内容確認_追加要素設定_SimNamespaceUri空.xml",               false)]
    [DataRow("ElementAddSetting_内容確認_追加要素設定_SimNamespaceXsdなし.xml",             true)] // SimNamespaceXsdがnullまたはEmptyはOK
    [DataRow("ElementAddSetting_内容確認_追加要素設定_SimNamespaceXsd空.xml",               true)] // SimNamespaceXsdがnullまたはEmptyはOK
    ...
    /// <summary>
    /// <see cref="ElementAddSetting.CheckGetElementSetting"/>のテスト（全体）
    /// </summary>
    ...
    public void CheckGetElementSettingTest_Overall(string keyName, int targetType, string targetValue, bool isBdgId, bool expectedCheckOK)
    {
        ...
        var method = typeof(ElementAddSetting).GetMethod("CheckGetElementSetting", BindingFlags.Instance | BindingFlags.NonPublic);
        var result = (string)method.Invoke(new ElementAddSetting(), ["取得要素設定", getSetting, isBdgId]);
    ...
    /// <summary>
    /// <see cref="ElementAddSetting.CheckGetElementSetting"/>のテスト（取得タイプに応じた値）
    /// </summary>
    /// <param name="targetType">取得タイプ</param>
    /// <param name="targetValue">取得タイプに応じた値</param>
    /// <param name="expectedCheckOK">期待値：正常かどうか</param>
    ...
    public void CheckGetElementSettingTest_TargetValue(int targetType, string targetValue, bool expectedCheckOK, string expectedTagName, string expectedAttributeName, string expectedAttributeValue, string expectedFixedValue)
    {
        ...
        var method = typeof(ElementAddSetting).GetMethod("CheckGetElementSetting", BindingFlags.Instance | BindingFlags.NonPublic);
        var result = (string)method.Invoke(new ElementAddSetting(), ["取得要素設定", getSetting, false]);
    ...
    /// <summary>
    /// <see cref="ElementAddSetting.CheckRemoveElementSetting"/>のテスト
    /// </summary>
    ...
    public void CheckRemoveElementSettingTest(string tagName, bool expectedCheckOK)
    {
        ...
        var method = typeof(ElementAddSetting).GetMethod("CheckRemoveElementSetting", BindingFlags.Instance | BindingFlags.NonPublic);
        var result = (string)method.Invoke(new ElementAddSetting(), [removeSetting]);
    ...
    /// <summary>
    /// <see cref="ElementAddSetting.CheckAddElementSetting"/>のテスト
    /// </summary>
    /// <param name="keyName">キー</param>
    /// <param name="tagName">要素名</param>
    /// <param name="expectedCheckOK">期待値：正常かどうか</param>
    ...
    public void CheckAddElementSettingTest(string keyName, string tagName, bool expectedCheckOK)
    {
        ...
        var method = typeof(ElementAddSetting).GetMethod("CheckAddElementSetting", BindingFlags.Instance | BindingFlags.NonPublic);
        var result = (string)method.Invoke(new ElementAddSetting(), [addSetting]);
